<?php
		 ...
		 */
			...
			if($i_customer!=''){
				...
				$product = $Product_info->get($rows['key_product']);
				
				if($product['amount'] == 0 && $product['subscription'] == 0){
					
					if(!date_null($rows['valid'])){
						$rows['valid'] = mysql_date_to_spanish($rows['valid']);
					}else{
						$rows['valid'] ='';
					}
					...
					if(!date_null($rows['next_payment'])){
						$rows['next_payment'] = mysql_date_to_spanish($rows['next_payment']);
					}else{
						$rows['next_payment'] ='';
					}
				}
			...
			$elements = array(
					...
					($session_vars['i_reseller']== 0)? 
					  ($product['amount'] == 0 && $product['subscription'] == 0) ? 
					  array('value'=>'','type'=>'calendar','label'=>'*Valido hasta','name'=>'valid'): 
					  array('value'=>'','type'=>'calendar','label'=>'*Proximo Pago','name'=>'next_payment') : null ,
					($session_vars['i_reseller']== 0) ? array('value'=>'0','type'=>'radio','label'=>'Activo','name'=>'active'):null,
					array('value'=>'0','type'=>'radio','label'=>'Debito Automatico','name'=>'automatic_debit'),
