<?php
		 ...
		 */
			...
			$Elements 	   				= new Elements($Contents,$tpl);
			   ...
			   $filter=array(
				...
				while($row=db::fetch_assoc($grid)){
					...
					$tpl->setCurrentBlock('list');
							...
							$tpl->setVariable('i_channel',$row['i_channel']);
						
							$tpl->setVariable('name',$row['name']);
						
							$category = $Category->get($row['i_category']);
							
							$tpl->setVariable('category_name',$category['name']);
							
							$tpl->setVariable('cdn_status',$row['cdn_status']);
							
							$Elements->iconactive('status',$row['i_channel'],$row['status'],'status');
								
							$Elements->icondelete('delete',$row['i_channel'],$row['trash']);
							
							$Elements->iconedit('edit',$row['i_channel'],'content');
							
							$Elements->iconactive('active_action',$row['i_channel'],$row['active']);
							
							$Elements->iconrestore('restore',$row['i_channel'],$row['trash']);
